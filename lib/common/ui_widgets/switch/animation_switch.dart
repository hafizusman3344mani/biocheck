...
const double _kTrackHeight = 27.0;
...
const double _kSwitchWidth = 80.0;
const double _kSwitchHeight = 30.0;
...
class _RenderXliveSwitch extends RenderConstrainedBox {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final double borderThickness = 1.5 + (_kTrackRadius - 1.5) ;
    Rect rect111 = new Rect.fromLTRB(50.0,0.0,146.0,0.0
    );
   // final Paint paint = Paint()..color = trackColor;
    final Paint paint = Paint()..shader = LinearGradient(
      colors:_value ?AppColors.parrotGreen:AppColors.redGradien,
    ).createShader(rect111);
    ...
    _thumbPainter.paint(
      canvas,
      Rect.fromLTRB(
        thumbLeft + (currentValue*1.05).abs() *10.0,
        thumbCenterY - _XliveThumbPainter.radius-0.9,
        thumbRight - (currentValue/4.9).abs()*12.0,
